...
(function (global) {
  ...
  // Canonical vibe list injected server-side (window.__VIBES__ set before this script).
  // Each entry: { id, label, color, emoji, traits: { adj, chip, getaway, stop, escape } }
  var _raw = (typeof window !== "undefined" && window.__VIBES__) || [];

  var VIBES = _raw.map(function (v) {
    return { id: v.id, label: v.label, color: v.color, emoji: v.emoji };
  });
   ...
   */
  var VIBE_TRAITS = (function () {
    var map = {};
    _raw.forEach(function (v) {
      if (v.traits) map[v.id] = v.traits;
    });
    return map;
  })();
