...
class AIDemoProvider(FlightProvider):
    ...
    async def search(self, query: SearchQuery) -> list[FlightOffer]:
        # Demo-fare invention removed: never ask the AI to make up prices.
        # Deterministic seeded skeletons only — prices are always hidden by
        # the fare-missing gate and replaced with real cached ranges in the UI.
        return await self._offline_fallback(query)

    async def _offline_fallback(self, query: SearchQuery) -> list[FlightOffer]:
        """Deterministic seeded route skeletons (prices never shown to users)."""
